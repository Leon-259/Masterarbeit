...
Über alle Paare von \(gtTraj\) und \(sysTraj\) wird die Anzahl der Fehler aufsummiert. \(IDFN\) ist die Anzahl aller \glsdisp{FN}{falsch negative} \gls{Assoziation}[en], \(IDFP\) ist die der \glsdisp{FP}{falsch positive} und \(IDEP\) ist die Anzahl der echt positiven \gls{Assoziation}[en]. Mit diesen Definitionen lässt sich dann die \textit{\gls{IDF1}-Metrik} berechnen. 
...
Die Abbildung \ref{fig:KonzeptAssHOTA} veranschaulicht dieses Konzept. Äquivalent zu der \gls{Detektion}[en] gilt \(\glsdisp{EPA}{EPA}\) ist die Menge aller \(Ass_{\gls{EP}}\), \(\glsdisp{FPA}{FPA}\) ist die Menge aller \(Ass_{\gls{FP}}\) und \(\glsdisp{FNA}{FNA}\) die der \(Ass_{\gls{FN}}\).
